...
class ComputeAEROopenvsp_PA(Group):
...
class _ComputeAEROopenvsp(OPENVSPSimpleGeometry):
    ...
    def setup(self):
        ...
        if self.options["low_speed_aero"]:
            ...
            self.add_output("data:aerodynamics:wing:low_speed:CL_vector_0_degree", shape=SPAN_MESH_POINT)
            ...
            self.add_output("data:aerodynamics:horizontal_tail:low_speed:CL_0_isolated")
            self.add_output(
            ...
            self.add_output("data:aerodynamics:horizontal_tail:cruise:CL_0_isolated")
            self.add_output("data:aerodynamics:horizontal_tail:cruise:induced_drag_coefficient")

            self.add_output("data:aerodynamics:wing:cruise:CL_vector", shape=SPAN_MESH_POINT)
            self.add_output("data:aerodynamics:wing:cruise:CL_vector_0_degree", shape=SPAN_MESH_POINT)
            self.add_output("data:aerodynamics:wing:cruise:chord_vector", shape=SPAN_MESH_POINT, units="m")
            self.add_output("data:aerodynamics:wing:cruise:Y_vector", shape=SPAN_MESH_POINT, units="m")
            self.add_output(
                "data:aerodynamics:wing:cruise:area_vector",
                shape=SPAN_MESH_POINT,
                units="m**2",
            )
    ...
    def compute(self, inputs, outputs):
        ...
        (
            ...
            cm_0_wing,
            y_vector_wing,
            area_vector_wing,  # added
            cl_vector_wing,
            cl_vector_wing_0_degrees,  # added
            chord_vector_wing,
            coef_k_wing,
            cl_0_htp,
            cl_X_htp,
        ...
        if self.options["low_speed_aero"]:
            ...
            outputs["data:aerodynamics:wing:low_speed:area_vector"] = area_vector_wing
            outputs["data:aerodynamics:wing:low_speed:CL_vector"] = cl_vector_wing
            outputs["data:aerodynamics:wing:low_speed:CL_vector_0_degree"] = cl_vector_wing_0_degrees
            ...
            outputs["data:aerodynamics:horizontal_tail:low_speed:CL_0_isolated"] = cl_0_htp_isolated
            ...
            outputs["data:aerodynamics:horizontal_tail:cruise:CL_0_isolated"] = cl_0_htp_isolated
            outputs[
                "data:aerodynamics:horizontal_tail:cruise:induced_drag_coefficient"
            ] = coef_k_htp
            outputs["data:aerodynamics:wing:cruise:CL_vector"] = cl_vector_wing
            outputs["data:aerodynamics:wing:cruise:CL_vector_0_degree"] = cl_vector_wing_0_degrees
            outputs["data:aerodynamics:wing:cruise:Y_vector"] = y_vector_wing
            outputs["data:aerodynamics:wing:cruise:area_vector"] = area_vector_wing
            outputs["data:aerodynamics:wing:cruise:chord_vector"] = chord_vector_wing

...
@oad.RegisterSubmodel(SUBMODEL_AIRCRAFT_Z_CG, "fastga.submodel.weight.cg.aircraft_empty.z.geometric")
class ComputeZCG(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        wing_configuration = inputs["data:geometry:wing_configuration"]
        ...
        if engine_layout == 1.0:
            # Engines on wing
            cg_engine = lg_height + height_max / 2.0 - wing_height
        else:
            cg_engine = lg_height + height_max/2.0
        cg_wing = lg_height + height_max / 2.0 - wing_height
        if wing_configuration == 3.0:
            #LG attached to fuselage
            cg_fuselage = lg_height + height_max / 2.0
        else:
            #LG attached to wing
            cg_fuselage = lg_height + wing_height

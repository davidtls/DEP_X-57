...
class ComputeCablesWeight(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("data:propulsion:electric_powertrain:cables:lsw", val=np.nan, units="kg/m")
        self.add_input("data:geometry:cabin:length", val=np.nan, units="m")

        self.add_output("data:weight:propulsion:electric_powertrain:cables:mass", units="kg")

        self.declare_partials("*", "*", method="fd")

    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):

        length_spec_wt = inputs["data:propulsion:electric_powertrain:cables:lsw"]
        cabin_length = inputs["data:geometry:cabin:length"]

        b4 = 2 * length_spec_wt * cabin_length  # mass has been multiplied by 2
        # to account for redundancy, this factor can be reduced as per the convenience of the designer

        outputs["data:weight:propulsion:electric_powertrain:cables:mass"] = b4

...
from src.fastga.models.performances.mission.promotions import list_v_lift_off_from_v2, list_vr_from_v2, list_simulate_takeoff
...
@RegisterOpenMDAOSystem("fastga.performances.mission.takeoff_E_PA", domain=ModelDomain.PERFORMANCE)
class TakeOffPhase(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem(
            ...
            promotes=list_v_lift_off_from_v2,
        ...
        self.add_subsystem(
            ...
            promotes=list_simulate_takeoff,
            #promotes=self.get_io_names(
            #    _simulate_takeoff(propulsion_id=self.options["propulsion_id"]),
...
class _v2(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("data:aerodynamics:wing:takeoff:CL_max_blown", val=np.nan)

        self.add_output("v2:speed", units="m/s")
        self.add_output("v2:angle", units="rad")
        self.add_output("v2:climb_gradient")

        self.add_output("cl_alpha_wing_blown_takeoff", units="rad**-1")
        self.add_output("cl0_blown")
        self.add_output("additional_CL_flaps_takeoff_blown")
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        cl_max_takeoff = inputs["data:aerodynamics:wing:takeoff:CL_max_blown"]
        ...
        while True:
            ...
            if climb_gradient < 0.0 and iteration_number == 0:
                print("Takeoff is not gonna be feasible")

            if climb_gradient > CLIMB_GRAD_AEO:
                break
            elif iteration_number < 30.0:
                if iteration_number < 1:
                    _LOGGER.critical("The thrust is {}".format(thrust))
                    _LOGGER.critical("The MTOW is {}".format(mtow))
                    _LOGGER.critical("The cd is {}".format(cd))
                    _LOGGER.critical("The cl is {}".format(cl))
                iteration_number += 1
                factor += 0.01
            elif not adjust_takeoff:
                _LOGGER.critical(
                    "Climb rate is less than {}, adjust weight, propulsion or takeoff configuration".format(
                        CLIMB_GRAD_AEO
                    )
                )
                _LOGGER.critical("Simulation continues with factor = 1.2 but this airplane does not accomplish "
                                 "required gradient.")
                factor = 1.2
                adjust_takeoff = True
            else:
        ...
        # Computing CL0, CL_alpha, d_CL_flaps at V = V2
        alpha1 = 0.0
        alpha2 = 10.0 * np.pi / 180
        Cl1 = WingBlownLift.compute_cl_cd_blown(inputs, thrust, alpha1, 0.5 * atm.density * v2 ** 2, "takeoff", True)[0]
        Cl2 = WingBlownLift.compute_cl_cd_blown(inputs, thrust, alpha2, 0.5 * atm.density * v2 ** 2, "takeoff", True)[0]
        cl_alpha = (Cl2 - Cl1) / (alpha2-alpha1)

        cl0 = WingBlownLift.compute_cl_cd_blown(inputs, thrust, 0.0, 0.5 * atm.density * v2 ** 2, "none", True)[0]
        delta_cl_takeoff = WingBlownLift.compute_cl_cd_blown(inputs, thrust, 0.0, 0.5 * atm.density * v2 ** 2,
                                                             "takeoff", True)[0] - cl0
        ...
        outputs["cl_alpha_wing_blown_takeoff"] = cl_alpha

        outputs["cl0_blown"] = cl0
        outputs["additional_CL_flaps_takeoff_blown"] = delta_cl_takeoff
...
class _v_lift_off_from_v2(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        for i in range(len(alpha)):
            ...
            v_lift_off[i] = fsolve(
                self.lift_off,
                np.array([45.0]),
                args=(inputs, thrust_rate, alpha[i], wing_area, atm_0.speed_of_sound, atm_0.density, mtow),
                xtol=1.0e-2,
            )
            ...
            while altitude_t < SAFETY_HEIGHT:
                ...
                cl = WingBlownLift.compute_cl_cd_blown(inputs, thrust, alpha_t, 0.5 * 1.225 * v_t ** 2,
                                                       "takeoff", True)[0]
    ...
    def lift_off(self, x, inputs, thrust_rate, alpha, wing_area, speed_of_sound, density, mtow):
        ...
        cl = WingBlownLift.compute_cl_cd_blown(inputs, thrust, alpha, 0.5 * density * V ** 2, "takeoff", True)[0]

        A = (-mtow * g + thrust * math.sin(alpha) + 0.5 * density * V ** 2 * wing_area * cl)
...
class _vr_from_v2(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input("data:weight:aircraft:MTOW", np.nan, units="kg")
        self.add_input("data:mission:sizing:takeoff:thrust_rate", np.nan)
        self.add_input("data:mission:sizing:takeoff:friction_coefficient_no_brake", val=0.015)
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        while (alpha_t != 0.0) and (v_t != 0.0):
            ...
            thrust = float(flight_point.thrust)
            # Calculate lift and drag
            cl = WingBlownLift.compute_cl_cd_blown(inputs, thrust, alpha_t, 0.5 * 1.225 * v_t ** 2, "takeoff", True)[0]
            lift = 0.5 * atm.density * wing_area * cl * v_t ** 2
...
class _simulate_takeoff(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        cl_max_blown_to = inputs["data:aerodynamics:wing:takeoff:CL_max_blown"]
        cd0 = (inputs["data:aerodynamics:aircraft:low_speed:CD0"] + inputs["data:aerodynamics:flaps:takeoff:CD"])
        coeff_k = inputs["data:aerodynamics:wing:low_speed:induced_drag_coefficient"]
        wing_area = inputs["data:geometry:wing:area"]
        wing_span = inputs["data:geometry:wing:span"]
        ...
        # FIXME Vs1 should be determined with takeoff configuration
        # vs1 = math.sqrt((mtow * g) / (0.5 * Atmosphere(0).density * wing_area * cl_max_clean))
        vs1 = math.sqrt((mtow * g) / (0.5 * Atmosphere(0).density * wing_area * cl_max_blown_to))
        ...
        while altitude_t < SAFETY_HEIGHT:
            ...
            if not climb:
                ...
                bat_energy_takeoff += propulsion_model.get_consumed_energy(flight_point, offtakes, TIME_STEP / 3600)
                ...
                bat_energy_init_climb += propulsion_model.get_consumed_energy(flight_point, offtakes, TIME_STEP / 3600)
            ...
            if (time.time() - t_start) > MAX_CALCULATION_TIME:
                raise Exception(
                    "This configuration cannot takeoff. Adjust weight, propulsion or takeoff configuration."
                    "This may occur with values of max_motor_shaft_power_to_weight below 7 W/kg"
                    "Time calculation duration for takeoff phase [{}s] exceeded!".format(MAX_CALCULATION_TIME)
                )

    ...
    def dynamic_equilibrium_climb(
        ...
        if len(previous_step) == 5:
            result = fsolve(
                ...
                args=(inputs, thrust, q, dvx_dt, dvz_dt, mass, flap_condition, low_speed, x_cg),
                xtol=1.0e-3,
            )
        else:
            result = fsolve(
    ...
    def equation_outer_climb(
        ...
        if low_speed:
            if flap_condition == "takeoff":
                cl_wing = 0  # inputs["data:aerodynamics:flaps:takeoff:CL"]
                cd0_flaps = inputs["data:aerodynamics:flaps:takeoff:CD"]
            elif flap_condition == "landing":
                cl_wing = 0  # inputs["data:aerodynamics:flaps:landing:CL"]

"""Simple module for complete mission.
   Trying to calculate CL CD with aerodynamics -- components -- slipstream_patterson -- Blown_lift"""
...
from fastga.models.performances.mission.takeoff_E_PA import SAFETY_HEIGHT, TakeOffPhase
from fastga.models.performances.mission.dynamic_equilibrium_PA import DynamicEquilibrium_E_PA
...
@RegisterOpenMDAOSystem("fastga.performances.mission_E_PA", domain=ModelDomain.PERFORMANCE)
...
class _compute_taxi(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        bat_energy_taxi_out = propulsion_model.get_consumed_energy(flight_point, offtakes, duration / 3600) / 1000
...
class _compute_climb(DynamicEquilibrium_E_PA):
    """
    ...
    def setup(self):
        ...
        self.add_output("data:mission:sizing:main_route:climb:battery_power_array", units="W")
        self.add_output("data:mission:sizing:main_route:climb:battery_time_array", units="h")
        self.add_output("data:mission:sizing:main_route:climb:battery_capacity_array", units="A*h")
        ...
        self.add_output("data:mission:sizing:main_route:climb:gradient_rad", units="rad")
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        # FIXME: time_step forced since we dont have climb_rate_sl (we dont impose gamma, it comes from imposed thrust)
        ...
        while altitude_t < cruise_altitude:
            ...
            flight_point = FlightPoint(altitude=altitude_t,
                                       ...
                                       thrust_is_regulated=False,      #: If False, propulsion should match thrust rate.
                                       thrust_rate=thrust_rate,        # Fixed by us = 0.9
                                       mass=mass_t,
                                       name='sizing:main_route:climb',
                                       battery_power=0,)


            self.complete_flight_point(flight_point, v_cas=v_cas)
        ...
        outputs["data:mission:sizing:main_route:climb:battery_power_array"] = battery_power_climb_array[-1]
        outputs["data:mission:sizing:main_route:climb:battery_time_array"] = climb_time[-1]
        outputs["data:mission:sizing:main_route:climb:battery_capacity_array"] = climb_capacity[-1]
...
class _compute_cruise(DynamicEquilibrium_E_PA):
    """
    The hypothesis of small alpha/gamma angles is done.
    """
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        previous_step = self.dynamic_equilibrium(
            inputs, 0.0, q, 0.0, 0.0, mtow, "none", previous_step[0:5]
        )
...
class _compute_descent(DynamicEquilibrium_E_PA):
    """
    ...
    def setup(self):
        ...
        self.add_input("data:aerodynamics:wing:clean:CL_max_blown", val=np.nan)
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        while altitude_t > 1.0:
            ...
            previous_step = self.dynamic_equilibrium(
                inputs, flight_point.gamma, q, dvx_dt, 0.0, mtow, "none", previous_step[0:5]
            )
            ...
            bat_energy_descent += propulsion_model.get_consumed_energy(flight_point, offtakes, time_step / 3600) / 1000

...
The method of Torenbeek is ignored and the HTP is sized with the
volume coefficient of the tail of the X-57.
"""
...
class _UpdateArea(HTPConstraints):
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        # TODO We ignore Torenbeek method. We simply use the values from X-57 for the tail volume of the HTP.
        # This will slightly over-size the HTP since W/S from X-57 will be hardly achieved
        wing_mac = inputs["data:geometry:wing:MAC:length"]
        lp_ht = inputs["data:geometry:horizontal_tail:MAC:at25percent:x:from_wingMAC25"]
        wing_area = inputs["data:geometry:wing:area"]

        coeff_vol_maxwell = (2.4527 * 4.4589) / (6.196 * 0.6492)
        outputs["data:geometry:horizontal_tail:area"] = coeff_vol_maxwell * wing_area * wing_mac / lp_ht

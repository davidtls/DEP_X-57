""" Computes the intakes for the fuel cells stacks and the sized heat exchanger subsystem in a electric propulsion
...
class ComputeIntakes(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        Method is based on reference data found here :
            ...
            Supplies air to the cooling system ('cooling_intake')
        """
        ...
        self.add_input("data:geometry:electric_powertrain:cooling_intake:nb_intakes", val=np.nan, units=None)
        self.add_input("data:propulsion:electric_powertrain:cooling_intake:free_stream_speed", val=np.nan, units='m/s')

        self.add_output("data:geometry:electric_powertrain:cooling_intake:length", units="mm")
        self.add_output("data:geometry:electric_powertrain:cooling_intake:width", units="mm")
        self.add_output("data:geometry:electric_powertrain:cooling_intake:depth", units="mm")
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        hex_area = inputs['data:geometry:electric_powertrain:hex:area']
        air_speed = inputs['data:propulsion:electric_powertrain:hex:air_speed']
        nb_cooling_intakes = inputs['data:geometry:electric_powertrain:cooling_intake:nb_intakes']
        free_stream_speed = inputs['data:propulsion:electric_powertrain:cooling_intake:free_stream_speed']
        ...
        outputs['data:geometry:electric_powertrain:cooling_intake:length'] = L_hex
        outputs['data:geometry:electric_powertrain:cooling_intake:width'] = w_hex
        outputs['data:geometry:electric_powertrain:cooling_intake:depth'] = d_hex
